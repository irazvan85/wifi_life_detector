  ...
  return (
    <div className="min-h-screen bg-zinc-950 text-zinc-300 font-sans selection:bg-green-900/50 p-3 sm:p-4 md:p-8 flex flex-col items-center">
      ...
      <header className="w-full max-w-6xl flex flex-col sm:flex-row justify-between items-start sm:items-center mb-4 sm:mb-8 gap-3 sm:gap-4">
        ...
        <div className="flex items-center gap-2 sm:gap-3 w-full sm:w-auto">
          ...
          <button
            ...
            disabled={status === 'initializing' || (!isSimulated && connectionState !== 'connected' && !isActive)}
      ...
      <ConnectionPanel
        ...
        onSerialBaudRateChange={setSerialBaudRate}
      ...
      {/* Simulated Data Warning — only shown in simulation mode */}
      {isSimulated && (
      ...
      {/* Hardware Connected Banner */}
      {!isSimulated && connectionState === 'connected' && (
        <div className="w-full max-w-6xl mb-4 sm:mb-6 bg-green-950/40 border border-green-600/50 rounded-lg p-3 sm:p-4 flex items-start gap-3">
          <Wifi className="w-5 h-5 shrink-0 mt-0.5 text-green-400" />
          <div className="text-xs sm:text-sm">
            <strong className="block text-green-400 font-medium mb-1 font-mono text-xs uppercase">
              ● Hardware Connected
            </strong>
            <span className="text-green-300/80">
              Receiving live CSI data from{' '}
              {connectionMethod === 'websocket' ? 'WebSocket' : 'USB Serial'} board.
              {boardInfo ? ` Board: ${boardInfo.boardModel}` : ''}{' '}
              Press <strong className="text-green-400">Start Monitor</strong> to begin analysis.
            </span>
          </div>
        </div>
      )}
        ...
        <div className="w-full max-w-6xl mb-4 sm:mb-6 bg-zinc-900/40 border border-zinc-800/50 rounded-lg p-3 sm:p-4 flex items-start gap-3">
          ...
          <div className="text-xs sm:text-sm text-zinc-400">
            ...
            <span className="text-zinc-500 text-xs mt-1 inline-block">
              {isSimulated
                ? <>Tap <strong className="text-green-400">Start Monitor</strong> to begin CSI analysis with simulated sensor data.</>
                : connectionState === 'connected'
                  ? <>Tap <strong className="text-green-400">Start Monitor</strong> to begin live CSI analysis.</>
                  : <>Select a connection method above and connect your board, then tap <strong className="text-green-400">Start Monitor</strong>.</>
              }
            </span>
          </div>
        </div>
